...
if ($primary_category) {
  echo '<p>Category: <a href="' .
    esc_url(get_term_link($primary_category)) .
    '">' .
...
if ($partner_universities && is_array($partner_universities)) {
  ...
  foreach ($partner_universities as $partner_id) {
    $partner = ycf_get_data_object("partner_universities", null, [
      "entry_id" => $partner_id,
    ]);
    if ($partner) {
...
if ($hero_image) {
  echo '<img src="' .
    ...
    '"
alt="' .
    esc_attr($hero_image["alt"]) .
    '"
width="' .
    esc_attr($hero_image["width"]) .
    '"
height="' .
    esc_attr($hero_image["height"]) .
    '"
/>';
}
...
if ($author) {
  ...
  if (!empty($author["social_links"])) {
    ...
    if (!empty($author["social_links"]["twitter"])) {
      echo '<a href="' .
        esc_url($author["social_links"]["twitter"]) .
        '">Twitter</a> ';
    }

    if (!empty($author["social_links"]["linkedin"])) {
      echo '<a href="' .
        esc_url($author["social_links"]["linkedin"]) .
        '">LinkedIn</a>';
    }
    ...
    switch ($block_type) {
      ...
      case "hero":
        ...
        if (!empty($section["background_image"])) {
          $bg_image = wp_get_attachment_image_url(
        ...
        if (!empty($section["overlay_opacity"])) {
          echo '<div style="opacity: ' .
            esc_attr($section["overlay_opacity"] / 100) .
            ';"></div>';
        }

        if (
          !empty($section["show_cta"]) &&
          is_array($section["cta_button"]) &&
          !empty($section["cta_button"]["url"])
        ) {
          echo '<a href="' .
            esc_url($section["cta_button"]["url"]) .
            '" class="button">';
      ...
      case "two_column":
        ...
        if (!empty($section["left_content"])) {
          echo '<div class="column">' .
            wp_kses_post($section["left_content"]) .
            "</div>";
        }

        if (!empty($section["right_content"])) {
          echo '<div class="column">' .
            wp_kses_post($section["right_content"]) .
            "</div>";
        }
      ...
      case "team_member":
        ...
        if (!empty($section["email"])) {
          echo '<p><a href="mailto:' .
            esc_attr($section["email"]) .
            '">' .
            esc_html($section["email"]) .
            "</a></p>";
        }
        ...
        if (!empty($section["social_media"])) {
          ...
          if (!empty($section["social_media"]["linkedin"])) {
            echo '<a href="' .
              esc_url($section["social_media"]["linkedin"]) .
              '">LinkedIn</a> ';
          }

          if (!empty($section["social_media"]["twitter"])) {
            echo '<a href="' .
              esc_url($section["social_media"]["twitter"]) .
              '">Twitter</a>';
          }
      ...
      case "testimonial":
        ...
        if (!empty($section["company"])) {
          $company = ycf_get_data_object("company", null, [
            "entry_id" => $section["company"],
          ]);
      ...
      case "cta":
        ...
        if (!empty($section["background_image"])) {
          $cta_bg = wp_get_attachment_image_url(
            $section["background_image"],
            "full",
          );
        ...
        if (!empty($section["video_url"])) {
          ...
          if ($video_url) {
            ...
            if (!empty($section["thumbnail"])) {
              $poster_url = wp_get_attachment_image_url(
                $section["thumbnail"],
                "full",
              );
...
download>';
      ...
      case "university_info":
        ...
        if (!empty($section["university"])) {
          $uni = ycf_get_data_object("university", null, [
            "entry_id" => $section["university"],
          ]);
          if ($uni) {
